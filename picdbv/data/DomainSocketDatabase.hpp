...
#ifndef DOMAINSOCKETDATABASE_HPP
#define DOMAINSOCKETDATABASE_HPP
...
class DomainSocketDatabase: public Database
{
  public:
    ...
    DomainSocketDatabase(const std::string& db, const std::string& socketFile = serverSocketFile);


    /** \brief destructor */
    virtual ~DomainSocketDatabase();
  ...
  private:
    /** \brief sends a request to the Unix domain socket server
     *
     * and returns the server's response on success, or empty string on failure.
     *
     * \param request  the request string
     * \return Returns the server's response in case of success.
     *         Returns empty string, if an error occurred.
     */
    std::string sendRequestToServer(const std::string& request) const;

    std::string db_name;
    std::string socket_file;
}; //class

#endif // DOMAINSOCKETDATABASE_HPP
